...
### Gemini CLI Adapter
- [ ] **Exa.ai** : rechercher "Gemini CLI hooks documentation 2025"
- [ ] Créer `src/adapters/gemini-cli.ts` — `GeminiCLIAdapter extends BaseAdapter`
- [ ] Layer 1 — Hooks : handler pour BeforeAgent, AfterAgent, BeforeTool, AfterTool
- [ ] Layer 2 — File watching : watcher sur `~/.gemini/` pour les outputs/logs
- [ ] Layer 3 — Process detection : scanner pour `gemini` binary
- [ ] Mapping events Gemini → AISnitch :
  ```
  ...
  ```
- [ ] `aisnitch setup gemini-cli` dans le setup command

### Codex CLI Adapter
- [ ] **Exa.ai** : rechercher "Codex CLI json output format OpenAI"
- [ ] Créer `src/adapters/codex.ts` — `CodexAdapter extends BaseAdapter`
- [ ] Layer 1 — Log watching : watcher sur `~/.codex/log/codex-tui.log` via chokidar
- [ ] Layer 2 — Process detection : scanner pour `codex` binary
- [ ] Parser les logs pour extraire les events (format à déterminer via recherche)
- [ ] Mapping events Codex → AISnitch

- [ ] Écrire tests :
  - [ ] Parse d'un event Gemini hook
  - [ ] Parse d'un log Codex
  - [ ] Mapping correct
- [ ] Vérifier `pnpm build` + `pnpm test`

## Critères de complétion

- [ ] Gemini adapter détecte et capture les events
- [ ] Codex adapter détecte et capture les events
- [ ] Mapping correct vers AISnitch events
- [ ] Tests passent
- [ ] Code documenté
