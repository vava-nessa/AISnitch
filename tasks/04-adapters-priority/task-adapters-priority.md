...
Implémenter le système d'adapters avec la classe abstraite `BaseAdapter`, puis les deux adapters prioritaires : **Claude Code** (hooks HTTP + JSONL watcher + process detection) et **OpenCode** (plugins + process detection, avec recherche ACP/SQLite pour la suite).
